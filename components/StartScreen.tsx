...
const StartScreen: React.FC<StartScreenProps> = ({ onModelFinalized, onError }) => {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            const errorMessage = getFriendlyErrorMessage(err, 'Falha ao criar modelo');
            // If it's an API permission error, pass it up to App.tsx to handle the API key reset flow
            if (errorMessage.includes("PERMISSION_DENIED") || errorMessage.includes("Requested entity was not found") || JSON.stringify(err).includes("403")) {
                if (onError) onError(err);
            } else {
                 setLocalError(errorMessage);
            }
            // Keep the user image but stop spinner
            // setUserImageUrl(null); // Optional: reset or keep user image? Better to keep so they can try again.
        } finally {
      ...
      {!userImageUrl ? (
          ...
          <div className="lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left">
            <div className="max-w-lg">
              ...
              <p className="mt-4 text-lg text-gray-600">
                Já se perguntou como uma roupa ficaria em você? Pare de adivinhar. Envie uma foto e veja por si mesmo. Nossa IA cria seu modelo pessoal, pronto para experimentar qualquer coisa.
              </p>
              ...
              <div className="flex flex-col items-center lg:items-start w-full gap-3">
                ...
                <p className="text-gray-500 text-sm">Selecione uma foto nítida de corpo inteiro. Fotos apenas do rosto também funcionam, mas corpo inteiro é preferível para melhores resultados.</p>
                <p className="text-gray-500 text-xs mt-1">Ao enviar, você concorda em não criar conteúdo prejudicial, explícito ou ilegal. Este serviço é para uso criativo e responsável apenas.</p>
        ...
        >
          <div className="md:w-1/2 flex-shrink-0 flex flex-col items-center md:items-start">
            ...
            <AnimatePresence>
              {generatedModelUrl && !isGenerating && !localError && (
                <motion.div
